# afton-works-site

Official AFTON website — fragrance, objects, perception, and records around what remains after scent.

## Current priority
Build and publish a lightweight temporary landing experience before the full personalization system.

## Source-of-truth documents
- [`AGENTS.md`](./AGENTS.md) — rules for Codex / collaborating agents
- [`docs/00_PROJECT_STATUS.md`](./docs/00_PROJECT_STATUS.md) — current stage and scope
- [`docs/01_CONCEPT.md`](./docs/01_CONCEPT.md) — visual concept
- [`docs/15_HINGE_AND_RULE_ARCHIVE_ANALYSIS.md`](./docs/15_HINGE_AND_RULE_ARCHIVE_ANALYSIS.md) — founder/work archive analysis
- [`docs/16_FOUNDER_OPERATING_SYSTEM.md`](./docs/16_FOUNDER_OPERATING_SYSTEM.md) — ownership and continuity doctrine
- [`docs/17_AFTON_VENTURE_ARCHITECTURE_AND_ROADMAP.md`](./docs/17_AFTON_VENTURE_ARCHITECTURE_AND_ROADMAP.md) — venture structure and execution roadmap
- [`docs/18_CROSS_AGENT_HANDOFF_2026-09-13.md`](./docs/18_CROSS_AGENT_HANDOFF_2026-09-13.md) — current cross-agent handoff
- [`docs/19_GIT_RECEIPT_FOUNDER_ARCHITECTURE.md`](./docs/19_GIT_RECEIPT_FOUNDER_ARCHITECTURE.md) — founder-architecture commit receipt
- [`docs/20_INSTAGRAM_ARCHIVE_TRANSITION_PLAN.md`](./docs/20_INSTAGRAM_ARCHIVE_TRANSITION_PLAN.md) — Hinge & Rule closure and UNSEEN_RATIO publishing plan
- [`docs/21_GIT_RECEIPT_INSTAGRAM_TRANSITION.md`](./docs/21_GIT_RECEIPT_INSTAGRAM_TRANSITION.md) — Instagram transition planning receipt
- [`docs/22_HINGE_CLOSING_EDITION_CONTACT_SHEET.md`](./docs/22_HINGE_CLOSING_EDITION_CONTACT_SHEET.md) — Hinge & Rule final nine-post candidate grid
- [`docs/23_UNSEEN_ISSUE01_CONTACT_SHEET.md`](./docs/23_UNSEEN_ISSUE01_CONTACT_SHEET.md) — UNSEEN_RATIO Issue 01 editorial sequence
- [`docs/24_GIT_RECEIPT_SOCIAL_CONTACT_SHEETS.md`](./docs/24_GIT_RECEIPT_SOCIAL_CONTACT_SHEETS.md) — archive ledger and contact-sheet receipt
- [`docs/25_HINGE_AND_RULE_PUBLIC_TRACE_REPORT.md`](./docs/25_HINGE_AND_RULE_PUBLIC_TRACE_REPORT.md) — public web trace, evidence analysis, and recovery plan
- [`research/hinge-and-rule-public-trace/SOURCE_LEDGER_2026-09-13.csv`](./research/hinge-and-rule-public-trace/SOURCE_LEDGER_2026-09-13.csv) — source-by-source public evidence ledger
- [`research/hinge-and-rule-public-trace/SEARCH_LOG_2026-09-13.md`](./research/hinge-and-rule-public-trace/SEARCH_LOG_2026-09-13.md) — query scope, coverage, limits, and evidence rules
- [`docs/26_GIT_RECEIPT_HINGE_PUBLIC_TRACE.md`](./docs/26_GIT_RECEIPT_HINGE_PUBLIC_TRACE.md) — public-trace research receipt and next gate
- [`docs/02_VISUAL_REFERENCE_PLAN.md`](./docs/02_VISUAL_REFERENCE_PLAN.md) — reference and image-generation plan
- [`docs/04_GENERATED_ASSET_INDEX.md`](./docs/04_GENERATED_ASSET_INDEX.md) — generated-image index
- [`docs/05_CONCEPT_SHEET_BATCH01.md`](./docs/05_CONCEPT_SHEET_BATCH01.md) — first concept sheet

## Core line
**Familiar, but not identifiable.**

## Founder line
**Build the concept, prove it in matter, retain the system, and stay through value capture.**
